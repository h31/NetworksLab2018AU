...
public:
    ...
    int to_bytes(char *buffer, uint16_t idx, std::map<std::string, uint16_t> *offsets);
};
...
public:
    ...
    ~dns_packet() {
        delete questions;
        delete questions_offsets;
        delete RRs;
    }

    Header header;
    std::vector<dns_question> * questions;
    std::map<std::string, uint16_t> * questions_offsets;
    std::vector<dns_response_packet> * RRs;
private:
    Header to_network_byte_order();
    ...
    int get_domain_name_from_offset(char *data, std::string &domain_name, uint16_t idx);
};
